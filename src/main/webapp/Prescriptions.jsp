...
<header class="container mt-4 p-5"  style="background-color: lightblue; border-radius: 30px;" >
    ...
    <div class="row bg-light m-1 p-3 my-2" style="border-radius: 10px; overflow: hidden; height: 150px;">
        <!-- Prescription Image -->
        <div class="col-3 d-flex justify-content-center align-items-center">
            <a target="_blank" href="GetPrescriptionImage?id=<%= prescription.get("id") %>">
                <img class="img-fluid" 
        ...
        <div class="col-9 d-flex flex-column justify-content-center">
            <p class="mb-1" style="font-size: 14px;">Email: <b><%= prescription.get("email") %></b></p>
            ...
            <div class="d-flex justify-content-start gap-2 mt-2">
                <% if (status.equalsIgnoreCase("pending")) { %>
                    ...
                    <form action="CartForUser.jsp" method="post" class="d-inline">
                        <input type="hidden" name="uemail" value="<%= prescription.get("email") %>" />
                        <input type="hidden" name="id" value="<%= prescription.get("id") %>" />
                        <button class="btn btn-sm btn-success px-3 py-1" style="font-size: 12px;">Add to Cart</button>
                    </form>
                 
                <% } else { %>
                    <!-- Already Done Message -->
                    <span class="btn btn-sm btn-secondary px-3 py-1" style="font-size: 12px; cursor: default;">Completed</span>
                <% } %>
            </div>
        </div>
    </div>
