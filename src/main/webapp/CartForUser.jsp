	...
	<header class="container mt-4 p-5"
		...
		<div class="row">
			<div class="col-sm m-2">
				<div class="">
					<a target="_blank" href="GetPrescriptionImage?id=<%=pid %>"> <img
						class="img-fluid" src="GetPrescriptionImage?id=<%= pid %>"
			...
			<div class="col-sm m-2">
				<div>
					...
					<form action="CartForUser.jsp" method="post">
						<div class="form-row">
							<div class="col-10">
								<input type="hidden" name="uemail" value="<%= uemail %>" /> <input
									type="hidden" name="id" value="<%= pid %>" /> <input
									class="form-control" type="text" name="name" maxlength="100"
									placeholder="Medicine/Product Name" required />
							</div>
							<div class="col-2">
            ...
            %>
                    ...
                    %>
							<form action="AddToCartByAdmin" method="post">
								...
								<input type="hidden" name="uemail" value="<%= uemail %>" /> <input
									type="hidden" name="id" value="<%= pid %>" />
								<button class="btn btn-sm btn-warning">Add to Cart</button>
							</form>
							<%
                        } else {
                    %>
							<label class="bg-danger text-white p-2">Currently Not
								Available</label>
							<%
                        }
                    %>
            ...
            %>
					<p class="p-2">
						<img height="50px" src="GetImage?name=<%= item_name %>"
							alt="Product Image"> <b><%= item_name %></b>
						<%
                ...
                %>
                    ...
                    total += price;
                %>
						Price: <b><%= price %></b> <a class="btn btn-danger btn-sm"
							href='RemoveFromCartByAdmin?id=<%= id %>'>Remove</a>
					</p>
					<hr />
					<%
                    }
