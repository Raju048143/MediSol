...
<body>
  ...
  <nav class="navbar navbar-expand-sm container">
	...
	<header class="container-fluid mt-4 p-5"  style="background-color: lightblue; border-radius: 30px;">
		<h4 >All Orders:</h4>
		<div class=" p-1 my-1">
			...
			%>
	  				<img height="30px" src="GetImage?name=<%=ii[0]%>"> &nbsp;&nbsp;<%=ii[0] %> 
			...
			%></td>
  				...
  				if(status.equalsIgnoreCase("Placed")){
  					%>
  						...
  						<a class="btn btn-sm btn-danger" href="ChangeOrderStatus?id=<%=order.get("id") %>&status=Cancel&page=AdminOrders">Cancel</a>
  						<a class="btn btn-sm btn-success" href="ChangeOrderStatus?id=<%=order.get("id") %>&status=Confirmed&page=AdminOrders">Confirm</a>
  					<%
  				}else if(status.equalsIgnoreCase("Confirmed")){
  					%>
						
						<a class="btn btn-sm btn-primary" href="ChangeOrderStatus?id=<%=order.get("id") %>&status=Delivered&page=AdminOrders">Delivered</a>
					<%
				}else{
					%>
	  			NA
